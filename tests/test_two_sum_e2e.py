...
PROBLEM_DIR = str(Path(__file__).resolve().parent.parent / "problems" / "0001_two_sum")


class TestTwoSumE2E(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_problem_meta_valid(self):
        self.assertEqual(self.meta["problem_id"], "0001_two_sum")
        self.assertEqual(self.meta["entry"]["class_name"], "Solution")
        self.assertEqual(self.meta["entry"]["method_name"], "twoSum")
        self.assertIn("array", self.meta["pattern_tags"])
        self.assertIn("hash_map", self.meta["pattern_tags"])
    ...
    def test_visual_solution_trace_has_expected_events(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("array_read", event_types)
            self.assertIn("hash_map_get", event_types)
            self.assertIn("hash_map_put", event_types)
            self.assertIn("answer_found", event_types)

    def test_visual_solution_trace_deterministic(self):
        """Same case run twice produces identical trace events."""
        import tempfile
        from pv.harness import run_case

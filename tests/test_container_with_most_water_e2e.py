...
PROBLEM_DIR = str(Path(__file__).resolve().parent.parent / "problems" / "0011_container_with_most_water")


class TestContainerWithMostWaterE2E(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_problem_meta_valid(self):
        self.assertEqual(self.meta["problem_id"], "0011_container_with_most_water")
        self.assertEqual(self.meta["entry"]["class_name"], "Solution")
        self.assertEqual(self.meta["entry"]["method_name"], "maxArea")
        self.assertIn("two_pointers", self.meta["pattern_tags"])
        self.assertIn("greedy", self.meta["pattern_tags"])
    ...
    def test_visual_solution_trace_has_expected_events(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("area_compute", event_types)
            self.assertIn("comparison_reason", event_types)
            self.assertIn("pointer_move", event_types)
            self.assertIn("return", event_types)
            self.assertIn("best_update", event_types)
